...
const MicroPop = (() => {
  ...
  const init = config => {
    ...
    // Group triggers by target popup
    const popupGroups = {}
    ...
    triggers.forEach(trigger => {
      ...
      attributesToCheck.forEach(attr => {
        ...
        if (!popupGroups[targetId]) popupGroups[targetId] = []
        popupGroups[targetId].push({
          element: trigger,
          role: attr
        })
      })
    ...
    // Initialize 1 popup instance per group
    Object.keys(popupGroups).forEach(targetId => {
      const group = popupGroups[targetId]
      const triggerElements = group.map(item => item.element)
      const role = group[0].role // Assume all triggers in the group have the same role
